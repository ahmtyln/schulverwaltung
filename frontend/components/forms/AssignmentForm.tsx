...
import { createAssignment, updateAssignment, fetchLessons, fetchMe, type LessonListItem, type MeDto } from "@/lib/api";
...
const schema = z.object({
  ...
  endTime: z.string().min(1, "Due date is required"),
...
const AssignmentForm = ({
  ...
  return (
    <form className="flex flex-col gap-6" onSubmit={onSubmit}>
      <h2 className="text-lg font-semibold">{type === "create" ? "New assignment" : "Update assignment"}</h2>
      ...
      <div className="flex flex-col gap-2 w-full md:w-1/4">
        ...
        <select
          ...
          {...register("lessonId")}
        >
          <option value={0}>Select lesson (teacher is assigned via lesson)...</option>
          {lessons.map((l) => (
            <option key={l.id} value={l.id}>
              {l.name} — {l.subjectName} / {l.className}
              {l.teacherName ? ` — Teacher: ${l.teacherName}` : ""}
            </option>
          ))}
        </select>
        {errors.lessonId && (
